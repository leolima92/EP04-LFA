...
O projeto se baseia em **expressões regulares**, que são utilizadas para descrever padrões pertencentes à classe das **linguagens regulares**.

Na teoria de Linguagens Formais, toda expressão regular possui um **autômato finito equivalente** capaz de reconhecer a mesma linguagem. Dessa forma, cada expressão regular definida neste projeto pode ser entendida como um pequeno reconhecedor responsável por identificar uma categoria específica de informação dentro da frase digitada pelo usuário.

Neste trabalho, cada categoria de informação foi modelada como uma linguagem regular independente. Por exemplo:
...
Também há um arquivo com exemplos de testes em [`docs/exemplos.md`](docs/exemplos.md).
